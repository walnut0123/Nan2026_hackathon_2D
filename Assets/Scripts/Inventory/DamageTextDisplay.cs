...
public class DamageTextDisplay : MonoBehaviour
{
    ...
    // Option 1 (kept for later, superseded below by option 3 - merge window):
    // random per-spawn spread so a burst of near-simultaneous hits doesn't stack every number
    // in the exact same spot.
    // [SerializeField] private Vector2 positionJitter = new Vector2(0.3f, 0.15f);

    // Option 3: hits landing on the same target are summed and shown as a single number
    // instead of one number per hit. Sliding window - every new hit pushes the flush time
    // back out by mergeWindow again, so a whole burst (e.g. 5 staggered cards) merges into one
    // number as long as no two consecutive hits are more than mergeWindow apart, regardless of
    // how long the burst runs in total.
    ...
    ///<summary>Queues a damage number above the target's head; hits within mergeWindow of each other are summed into one display.</summary>
    public static void ShowDamage(int amount, Transform target)
    {
        if (Instance == null || Instance.damageAnimation == null || target == null)
            return;

        Instance.AccumulateDamage(amount, target);
    }
    ...
    {
        // Keep sleeping until mergeWindow seconds have passed since the MOST RECENT hit
        // (if a new hit pushes lastHitTime forward while we're asleep, loop and wait again) -
        // OR until maxMergeWindow has passed since the FIRST hit, whichever comes first, so a
        // continuous stream of hits still displays periodically instead of never flushing.
        ...
        // Target was destroyed while the window was open - no position left to show it at.
        if (target == null)
            yield break;
    ...
    // Option 1's jitter helper (kept for later, see field comment above):
